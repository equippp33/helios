...
import Image from 'next/image'; // ✅ If you want to use next/image for optimization
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: React.Key) => {
    ...
    const content = isNodeItem ? (
     ...
     <Image
       ...
       src={item.src}
       sizes={item.sizes}
       width={item.width ?? logoHeight}  // fallback to some number if undefined
       height={item.height ?? logoHeight} // fallback
       alt={item.alt ?? ''}
       title={item.title}
       loading="lazy" // optional, default is lazy unless priority=true
     />
    );
